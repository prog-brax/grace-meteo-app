...
onMounted(async () => {
  const meteo = await axios.get(
    "https://api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,uv_index_clear_sky_max&current=weather_code"
  );
  const etat = meteo.data.current.weather_code;
  const T1 = meteo.data.current.temperature_2m_min;
  const T2 = meteo.data.current.temperature_2m_max;
